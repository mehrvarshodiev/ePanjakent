...
function App() {
  ...
  return <div className="app-shell">
    <header className="topbar"><div className="nav-wrap">
      ...
      <nav className="desktop-nav"><a href="#news">Хабарҳо</a><a href="#tourism">Туризм</a><a href="#services">Хизматҳо</a><a href="#digital">Рақамӣ</a><a href="#appeals">Муроҷиатҳо</a></nav>
    ...
    </div>{menu && <div className="mobile-menu"><a href="#news" onClick={() => setMenu(false)}>Хабарҳо</a><a href="#tourism" onClick={() => setMenu(false)}>Туризм</a><a href="#services" onClick={() => setMenu(false)}>Хизматҳо</a><a href="#digital" onClick={() => setMenu(false)}>Рақамӣ</a><a href="#appeals" onClick={() => setMenu(false)}>Муроҷиатҳо</a></div>}</header>

    <main>
      <section className="hero"><div className="hero-orb orb-one"/><div className="hero-orb orb-two"/><div className="hero-grid"/><div className="hero-content reveal"><div className="eyebrow"><span/> PANJAKENT · TAJIKISTAN</div><h1>PANJAKENT<br/><em>— DIGITAL CITY</em></h1><p>Шаҳри таърих ва оянда. Платформаи ягонаи рақамии Панҷакент барои хизматрасонӣ, ахбор, сайёҳӣ ва иштироки шаҳрвандон.</p><div className="hero-buttons"><a href="#services" className="btn btn-gold">Хизматрасониҳо <span>→</span></a><a href="#tourism" className="btn btn-glass">Роҳнамои шаҳр <span>⌖</span></a></div></div><div className="scroll-hint">SCROLL TO EXPLORE ↓</div></section>
      <section id="news" className="section reveal"><SectionHeader number="01" label="NEWS BLOG" title="Блоги хабарҳо" link="Ҳама →"/><div className="filters"><button className="active">Featured</button><button>News</button><button>Culture</button><button>Events</button></div><div className="news-grid">{news.map((item, i) => <article className={`news-card ${i === 0 ? 'featured' : ''}`} key={item.title}><div className="card-image"><img src={item.image} alt=""/><span>{item.tag}</span></div><div className="card-body"><small>29 АВГУСТ 2026</small><h3>{item.title}</h3><p>{item.text}</p><a href="#">Хондан <span>↗</span></a></div></article>)}</div></section>
      <section id="services" className="services-section"><div className="section reveal"><SectionHeader number="02" label="E-GOVERNMENT" title="Хизматрасониҳои давлатӣ"/><div className="service-grid">{services.map(([n,title,text,icon]) => <article className="service-card" key={title}><div className="service-top"><span className="number">{n}</span><span className="service-icon">{icon}</span></div><h3>{title}</h3><p>{text}</p><button>Оғоз кардан <span>→</span></button></article>)}</div></div></section>
      <section id="tourism" className="section reveal"><SectionHeader number="03" label="EXPLORE" title="Роҳнамои туристии Панҷакент"/><div className="places-grid">{places.map(([n,title,tag,image]) => <article className="place-card" key={title}><img src={image} alt={title}/><div className="place-overlay"/><div className="place-top"><span>{n}</span><span>{tag}</span></div><div className="place-bottom"><h3>{title}</h3><button>Кашф кардан →</button></div></article>)}</div></section>
      <section id="digital" className="digital"><div className="section digital-inner reveal"><SectionHeader number="04" label="DIGITAL PANJAKENT" title="Панҷакенти рақамӣ" light/><p className="digital-lead">Технология, инфрасохтор ва ташаббусҳои smart city барои рушди устувори шаҳр.</p><div className="metrics"><Metric value="+12%" label="Growth"/><Metric value="48" label="Schools"/><Metric value="87%" label="Digital"/><Metric value="126" label="Projects"/></div><div className="chart"><div><b>Индекси рушди рақамӣ</b><span>2015 — 2026 ↗</span></div><div className="bars">{[25,32,30,39,48,44,57,64,61,75,86,100].map((h,i)=><i style={{height:`${h}%`}} key={i}/>)}</div></div></div></section>
      <section id="appeals" className="section reveal"><div className="appeal"><div><div className="eyebrow gold">05 / CIVIC ENGAGEMENT</div><h2>Шаҳри худро<br/><em>якҷоя беҳтар мекунем.</em></h2><p>Масъалаҳои маҳаллиро хабар диҳед, пешниҳод фиристед ва ҳолати муроҷиататонро онлайн пайгирӣ кунед.</p></div><div className="appeal-actions"><button className="btn btn-gold">Муроҷиат фиристед →</button><button className="btn btn-outline">Дидани масъалаҳо</button></div></div></section>
    </main>
