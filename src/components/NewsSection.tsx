...
export default function NewsSection({ compact = false }: { compact?: boolean }) {
  ...
  return <section className={`section reveal visible news-section ${compact ? 'news-home-section' : ''}`}>
    ...
    <div className="news-grid">{items.map((item) => <article className="news-card" key={`${item.date}-${item.title}`}><img src={item.image} alt={item.title} loading="lazy"/><div className="news-card-body"><small>{item.tag} · {item.date}</small><h3>{item.title}</h3><p>{item.text}</p><Link to="/news" className="read-more">Муфассал →</Link></div></article>)}</div>
  </section>
}
